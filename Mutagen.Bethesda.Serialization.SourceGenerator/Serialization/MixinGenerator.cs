...
public class MixinGenerator
{
    ...
    public void Initialize(
        ...
        IncrementalValueProvider<ImmutableDictionary<LoquiTypeSet, RecordCustomizationSpecifications>>
            recordCustomizationDriver)
    {
        ...
        ImmutableHashSet<INamedTypeSymbol>? metaObjs)
    {
        ...
        if (!_serializationNaming.TryGetSerializationItems(bootstrap.ObjectRegistration, out var modSerializationItems))
            return;
            ...
            new string[]
                {
                    ...
                    $"Mutagen.Bethesda.Serialization.Exceptions",
        ...
        using (sb.CurlyBrace())
        {
            ...
            sb.AppendLine(
                $"private readonly static MutagenSerializationWriterKernel<{writerKernel}, {writer}> WriterKernel = new();");
            sb.AppendLine();
            ...
            using (sb.CurlyBrace())
            {
                ...
                {
                    sb.AppendLine(
                        "using var streamCreatorDispose = SerializationHelper.GetStreamCreator(streamCreator, fileSystem, path, out streamCreator);");
                }
                else
                {
                    sb.AppendLine(
                        "streamCreator = streamCreator.GetOrFallback(static () => NormalFileStreamCreator.Instance);");
                }

                if (customization.FilePerRecord)
                {
                    sb.AppendLine(
                        "var exceptionPath = Path.Combine(path, SerializationHelper.RecordDataFileName(ReaderKernel.ExpectedExtension));");
                    sb.AppendLine(
                        "using var streamPassIn = streamCreator.GetStreamFor(fileSystem, exceptionPath, write: true);");
                }
                else
                {
                    sb.AppendLine("var exceptionPath = path;");
                    sb.AppendLine(
                        "using var streamPassIn = streamCreator.GetStreamFor(fileSystem, path, write: true);");
                }
                ...
                using (sb.CurlyBrace())
                {
                    ...
                    if (isMod)
                    {
                        ...
                        {
                            sb.AppendLine(
                                "await WriterKernel.WriteLoqui(writer, extraMeta.GetType().Name, extraMeta, new SerializationMetaData(cancel.Value), metaWriter);");
                        }
                    }

                    sb.AppendLine(
                        $"await {modSerializationItems.SerializationCall()}<{writerKernel}, {writer.Name}>(writer, item, WriterKernel, workDropoff, fileSystem, streamCreator, cancel.Value);");
                ...
                foreach (var metaObj in metaObjs)
                {
                    ...
                    using (sb.CurlyBrace())
                    {
                        using (var c = sb.Call("await SerializeInternal"))
                        {
                            ...
                            c.Add(
                                $"metaWriter: static (w, m, k, s) => {metaSerializationObj.SerializationCall()}(w, m, k, s)");
                            c.AddPassArg("cancel");
                        }
                    }
            ...
            using (var args = sb.Function(
                       $"private static async Task<{bootstrap.ObjectRegistration.ContainingNamespace}.{names.Setter}> DeserializeInternal<TMeta>"))
            {
                args.Add($"this {bootstrap.Bootstrap} converterBootstrap");
                ...
                if (isMod)
                {
                    ...
                    args.Add("TMeta? extraMeta = default");
                    args.Add(
                        $"ReadInto<ISerializationReaderKernel<{reader.Name}>, {reader.Name}, TMeta>? metaReader = null");
                }
                else
                {
            ...
            using (sb.CurlyBrace())
            {
                ...
                if (isMod)
                {
                    using (var c = sb.Call("SerializationHelper.ExtractMeta"))
                    {
                        ...
                        c.Add(
                            $"path: {(customization.FilePerRecord ? $"Path.Combine(path, SerializationHelper.RecordDataFileName(ReaderKernel.ExpectedExtension))" : "path")}");
                        c.AddPassArg("streamCreator");
                ...
                if (customization.FilePerRecord)
                {
                    sb.AppendLine(
                        "var exceptionPath = Path.Combine(path, SerializationHelper.RecordDataFileName(ReaderKernel.ExpectedExtension));");
                    sb.AppendLine(
                        "using var streamPassIn = streamCreator.GetStreamFor(fileSystem, exceptionPath, write: false);");
                }
                else
                {
                    sb.AppendLine("var exceptionPath = path;");
                    sb.AppendLine(
                        "using var streamPassIn = streamCreator.GetStreamFor(fileSystem, path, write: false);");
                }
                ...
                using (sb.CurlyBrace())
                {
                    ...
                    using (var c = sb.Call($"return await {modSerializationItems.DeserializationCall()}{readerGen}"))
                    {
                        ...
                        if (isMod)
                        {
                            ...
                            {
                                c.Add(
                                    $"release: release.To{_releaseRetriever.GetReleaseName(bootstrap.ObjectRegistration)}Release()");
                            }
                    ...
                    using (sb.CurlyBrace())
                    {
                        using (var c = sb.Call("return await DeserializeInternal"))
                        {
                            ...
                            if (isMod)
                            {
                                ...
                                c.Add(
                                    $"metaReader: static (r, m, k, s) => {metaSerializationObj.DeserializationIntoCall()}(r, k, m, s)");
                            }
                            else
                            {
                                c.AddPassArg("metaData");
            ...
            using (sb.CurlyBrace())
            {
                ...
                if (customization.FilePerRecord)
                {
                    sb.AppendLine("var exceptionPath = Path.Combine(path, $\"Data{ReaderKernel.ExpectedExtension}\");");
                    sb.AppendLine(
                        "using var streamPassIn = streamCreator.GetStreamFor(fileSystem, exceptionPath, write: false);");
                }
                else
                ...
                using (sb.CurlyBrace())
                {
                    var pathStreamPassAlong = customization.FilePerRecord
                        ...
                        : "new StreamPackage(streamPassIn, Path.GetDirectoryName(path)!)";
                    using (var c = sb.Call(
                               $"await {modSerializationItems.DeserializationIntoCall()}<{reader.Name}, object>"))
                    {
        ...
        context.AddSource($"{bootstrap.Bootstrap.Name}_{modSerializationItems.TermName}_MixIns.g.cs",
