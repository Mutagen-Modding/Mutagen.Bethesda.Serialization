...
internal static class SomeObject_Serialization
{
        ...
        ISerializationReaderKernel<TReadObject> kernel)
    {
        while (kernel.TryGetNextField(reader, out var name))
        {
            ...
            {
                case "SomeMember0":
                    item.SomeMember0 = kernel.ReadP3UInt8(reader);
                case "SomeMember1":
                    item.SomeMember1 = kernel.ReadP3UInt8(reader);
                case "SomeMember2":
                    item.SomeMember2 = kernel.ReadP3UInt8(reader);
                case "SomeMember3":
                    item.SomeMember3 = kernel.ReadP3UInt8(reader);
                case "SomeMember4":
                    item.SomeMember4 = kernel.ReadP3UInt8(reader);
                case "SomeMember5":
                    item.SomeMember5 = kernel.ReadP3UInt8(reader);
                default:
                    break;
            }
        }
