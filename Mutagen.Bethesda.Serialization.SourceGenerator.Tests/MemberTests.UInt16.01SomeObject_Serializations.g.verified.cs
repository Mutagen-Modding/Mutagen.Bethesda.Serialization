...
internal static class SomeObject_Serialization
{
        ...
        ISerializationReaderKernel<TReadObject> kernel)
    {
        while (kernel.TryGetNextField(reader, out var name))
        {
            ...
            {
                case "SomeMember0":
                    item.SomeMember0 = kernel.ReadUInt16(reader);
                case "SomeMember1":
                    item.SomeMember1 = kernel.ReadUInt16(reader);
                case "SomeMember2":
                    item.SomeMember2 = kernel.ReadUInt16(reader);
                case "SomeMember3":
                    item.SomeMember3 = kernel.ReadUInt16(reader);
                case "SomeMember4":
                    item.SomeMember4 = kernel.ReadUInt16(reader);
                case "SomeMember5":
                    item.SomeMember5 = kernel.ReadUInt16(reader);
                case "SomeMember6":
                    item.SomeMember6 = kernel.ReadUInt16(reader);
                case "SomeMember7":
                    item.SomeMember7 = kernel.ReadUInt16(reader);
                case "SomeMember8":
                    item.SomeMember8 = kernel.ReadUInt16(reader);
                case "SomeMember9":
                    item.SomeMember9 = kernel.ReadUInt16(reader);
                case "SomeMember10":
                    item.SomeMember10 = kernel.ReadUInt16(reader);
                case "SomeMember11":
                    item.SomeMember11 = kernel.ReadUInt16(reader);
                default:
                    break;
            }
        }
