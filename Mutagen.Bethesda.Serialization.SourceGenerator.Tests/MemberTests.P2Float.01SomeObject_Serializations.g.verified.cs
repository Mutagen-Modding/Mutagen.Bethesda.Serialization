...
internal static class SomeObject_Serialization
{
        ...
        ISerializationReaderKernel<TReadObject> kernel)
    {
        while (kernel.TryGetNextField(reader, out var name))
        {
            ...
            {
                case "SomeMember0":
                    item.SomeMember0 = kernel.ReadP2Float(reader);
                case "SomeMember1":
                    item.SomeMember1 = kernel.ReadP2Float(reader);
                case "SomeMember2":
                    item.SomeMember2 = kernel.ReadP2Float(reader);
                case "SomeMember3":
                    item.SomeMember3 = kernel.ReadP2Float(reader);
                case "SomeMember4":
                    item.SomeMember4 = kernel.ReadP2Float(reader);
                case "SomeMember5":
                    item.SomeMember5 = kernel.ReadP2Float(reader);
                default:
                    break;
            }
        }
