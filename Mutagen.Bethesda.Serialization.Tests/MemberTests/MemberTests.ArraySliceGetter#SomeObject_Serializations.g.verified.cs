...
internal static class SomeObject_Serialization
{
        ...
        where TWriteObject : IContainStreamPackage
    {
            ...
            && checkedSomeArray.Length > 0)
        {
            ...
            {
                kernel.WriteFloat(writer, null, listItem, default(float), checkDefaults: false);
            }
        ...
        where TReadObject : IContainStreamPackage
    {
        ...
        switch (name)
        {
            case "SomeArray":
                {
                    obj.SomeArray = await SerializationHelper.ReadArray<ISerializationReaderKernel<TReadObject>, TReadObject, float>(
                        reader: reader,
                        kernel: kernel,
                        metaData: metaData,
                        itemReader: static async (r, k, m) =>
                        {
                            return SerializationHelper.StripNull(k.ReadFloat(r), name: "SomeArray");
                        });
                }
                break;
            case "SomeArray2":
                {
                    obj.SomeArray2 = await SerializationHelper.ReadArray<ISerializationReaderKernel<TReadObject>, TReadObject, float>(
                        reader: reader,
                        kernel: kernel,
                        metaData: metaData,
                        itemReader: static async (r, k, m) =>
                        {
                            return SerializationHelper.StripNull(k.ReadFloat(r), name: "SomeArray2");
                        });
                }
                break;
            case "SomeArray3":
                {
                    await SerializationHelper.ReadIntoArray<ISerializationReaderKernel<TReadObject>, TReadObject, float>(
                        reader: reader,
                        arr: obj.SomeArray3,
                        kernel: kernel,
                        metaData: metaData,
                        itemReader: static async (r, k, m) =>
                        {
                            return SerializationHelper.StripNull(k.ReadFloat(r), name: "SomeArray3");
                        });
                }
                break;
            default:
